...
public class DoingActivity extends Activity {
	...
	// Variable die de huidige activiteit bijhoudt die de gebruiker aan het doen is.
	...
	/**
	 * Methode die de opgeslagen methodes uit de database ophaalt
	 * en ze weergeeft op het scherm.
	 */
	...
	private void createCategoryButton(String category) {
		...
		newButton.setTextColor(getBaseContext().getResources().getColor(R.color.White));
		newButton.setBackgroundResource(R.layout.doing_activity_button_background);
	...
	private OnClickListener addButtonClickListener = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			DATABASE.open();
			boolean categoryAlreadyExists = Arrays.asList(DATABASE.readActivities()).contains(newCategory);
